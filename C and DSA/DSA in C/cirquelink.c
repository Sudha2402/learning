/*
	...
	//circular queue linkedlist means circular linkedlist and then queue implementation in that.
*/
...
struct CircularQueue//global to let it use inside functions also//here we need space to store front and rear value and earlier we strated head as front node hence we used traversal to reach to the last node but here we can directly move to rear node with the help of external pointer
{
	struct QNode *front;
	struct QNode *rear;
...
void printData(struct CircularQueue *q)
{
	...
	printf("\n Queue Element \n");
	while (node != NULL)//we could have used do while too with print f at first do then next time when node next is front then stop but here we have not connected rear node to front hence only this while can be used
	{
