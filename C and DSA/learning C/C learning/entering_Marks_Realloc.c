...
int main()
{
    ...
    if (marks == NULL) {
        ...
        printf("Memory has been successfully allocated by "
               "using malloc\n");
        printf("\n marks = %pc\n",marks);// marks = 007619E8c // print the base or beginning address of allocated memory //%p format specifier is used to write the address in  007619E8c form
        do {
            printf("\n Enter Marks\n");
            ...
            if (ans == 1) {
                ...
                marks = (int*)realloc(marks,
                    (index + 1)* sizeof(int)); // Dynamically reallocate// memory by using realloc
