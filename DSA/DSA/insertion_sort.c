...
void insertionSort2(int arr[], int size)//either way is fine//[] square bracket is used
{
    ...
    for (i = 1; i < size - 1; i++)
    {
        ...
        for (j = i-1; j >= 0; j--)
        {
            ...
            //this will always move the j to the very first position of iteration that is 0 and then at outside this will be inserted at j=0 position not at the position where key is smaller. hence use conditions to decrements j by using while loop
        }
