...
if (!$conn->connect_error) {
    ...
    if (isset($_POST["productCode"])) {
        ...
        // Replace "X" with "m":
// SELECT REPLACE("XYxZ FGH XYZ", "X", "m");

//all ordered products


        // $stmt0=$conn->prepare("SELECT orders.orderNumber as orderNumber,order_details.productCode,orders.customerNumber,orders.ratingOfStars,orders.comments,orders.status from products,orders,order_details where (products.productCode = order_details.productCode AND order_details.orderNumber=orders.orderNumber and order_details.productCode !='cartItems' and orders.status='delivered'  and order_details.modeOfPayment != 'cart' and orders.ratingOfStars IS NOT NULL and order_details.productCode = '$productCode') UNION All SELECT cartplaceorder.orderNumberCart as orderNumber,cartplaceorder.productCode,orders.customerNumber,cartplaceorder.ratingOfStars,cartplaceorder.comments ,cartplaceorder.status FROM cartplaceorder, products,orders,order_details  where (products.productCode = cartplaceorder.productCode AND order_details.orderNumber=orders.orderNumber AND  orders.orderNumber=cartplaceorder.orderNumber and cartplaceorder.modeOfPayment != 'cart'  and order_details.productCode ='cartItems' and cartplaceorder.status='delivered' and cartplaceorder.ratingOfStars  IS NOT NULL and cartplaceorder.productCode = '$productCode')");


// SELECT orders.orderNumber as orderNumber,order_details.productCode,orders.customerNumber,orders.ratingOfStars,orders.comments,orders.status from products,orders,order_details where (products.productCode = order_details.productCode AND order_details.orderNumber=orders.orderNumber and order_details.productCode !='cartItems' and orders.status='delivered'  and order_details.modeOfPayment != 'cart' and orders.ratingOfStars IS NOT NULL and order_details.productCode = 'WEWL1') UNION All SELECT cartplaceorder.orderNumberCart as orderNumber,cartplaceorder.productCode,orders.customerNumber,cartplaceorder.ratingOfStars,cartplaceorder.comments ,cartplaceorder.status FROM cartplaceorder, products,orders,order_details  where (products.productCode = cartplaceorder.productCode AND order_details.orderNumber=orders.orderNumber AND  orders.orderNumber=cartplaceorder.orderNumber and cartplaceorder.modeOfPayment != 'cart'  and order_details.productCode ='cartItems' and cartplaceorder.status='delivered' and cartplaceorder.ratingOfStars  IS NOT NULL and cartplaceorder.productCode = 'WEWL1');

//it returns all records

// we need customers profile, customername from customerdata,
...
// $reviewDetails = array();//for array
// //we want each row from $review into $reviewDetails array
// // foreach ($review  as $row) {
// foreach ($result0->fetch_assoc() as $row) {
    // $reviewDetails[] = $row;
    // $reviewDetails[] = $review ;
// }

// echo json_encode($reviewDetails);//it will 
// echo json_encode($reviewDetails);//it will
...
// echo json_encode(array( $review));//it will work alone but don't forget to use $result0->fetch_all(MYSQLI_ASSOC); instead of $result0->fetch_all() will return only one row
...
// $stmt = $conn->prepare("SELECT orders.ratingOfStars,count(*) from products,orders,order_details where (products.productCode = order_details.productCode AND order_details.orderNumber=orders.orderNumber and order_details.productCode !='cartItems' and orders.status='delivered'  and order_details.modeOfPayment != 'cart' and orders.ratingOfStars IS NOT NULL and order_details.productCode = '$productCode' GROUP BY ratingOfStars UNION All SELECT cartplaceorder.ratingOfStars,count(*) FROM FROM cartplaceorder, products,orders,order_details  where (products.productCode = cartplaceorder.productCode AND order_details.orderNumber=orders.orderNumber AND  orders.orderNumber=cartplaceorder.orderNumber and cartplaceorder.modeOfPayment != 'cart'  and order_details.productCode ='cartItems' and cartplaceorder.status='delivered' and cartplaceorder.ratingOfStars  IS NOT NULL and cartplaceorder.productCode = '$productCode' GROUP BY ratingOfStars;");
        ...
        // select ratingOfStars,count(*) from order_details,orders WHERE order_details.orderNumber=orders.orderNumber and productCode='WEWKS1' and ratingOfStars='5' GROUP BY ratingOfStars;

        // $stmt2 = $conn->prepare("SELECT ratingOfStars,count(*) from order_details,orders WHERE order_details.orderNumber=orders.orderNumber and productCode='$productCode' and ratingOfStars=5 GROUP BY ratingOfStars;");

        // $stmt5 = $conn->prepare("SELECT IFNULL(count(*),0) as 'count(*)' from order_details,orders WHERE order_details.orderNumber=orders.orderNumber and productCode='$productCode' and ratingOfStars=5 GROUP BY ratingOfStars;");//group by is creating problem with the warning Warning - Trying to access array offset on value of type null//no need of ifnull
        // $stmt5 = $conn->prepare("SELECT IFNULL(count(*),0) as 'count(*)' from order_details,orders WHERE order_details.orderNumber=orders.orderNumber and productCode='$productCode' and ratingOfStars=5 ;");

        // $stmt5 = $conn->prepare("SELECT count(*) as 'count(*)' from order_details,orders WHERE order_details.orderNumber=orders.orderNumber and productCode='$productCode' and ratingOfStars=5 ;");//works

        // $stmt5 = $conn->prepare("SELECT count(*) as 'count(*)' from $review WHERE order_details.orderNumber=orders.orderNumber and productCode='$productCode' and ratingOfStars=5 ;");//array to sting conversion error
...
// to remove this warning Warning - Trying to access array offset on value of type null

        // if (is_null($totalNumberOfReview["count(*)"]) ) {
        //     $totalNumberOfReview["count(*)"] = 0;
        // }
        // if (is_null($star1["count(*)"])) {
        //     $star1["count(*)"] = 0;
        // }
        // if (is_null($star2["count(*)"])) {
        //     $star2["count(*)"] = 0;
        // }
        // if (is_null($star3["count(*)"] )) {
        //     $star3["count(*)"] = 0;
        // }
        // if (is_null($star4["count(*)"]) ){
        //     $star4["count(*)"] = 0;
        // }
        // if (is_null($star5["count(*)"] )) {
        //     $star5["count(*)"] = 0;
        // }
        
        // if (is_null($sumOfReview["SUM(ratingOfStars)"])) {
        //     $sumOfReview["SUM(ratingOfStars)"] = 0;
        // }
        ...
        //  {"totalNumberOfReview":5,"star1":null,"star2":1,"star3":2,"star4":2,"star5":null,"sumOfReview":"16"}
        ...
        // {"totalNumberOfReview":{"count(*)":5},"star1":null,"star2":{"count(*)":1},"star3":{"count(*)":2},"star4":{"count(*)":2},"star5":null,"sumOfReview":{"SUM(ratingOfStars)":"16"}}
